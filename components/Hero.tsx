...
const FragmentShader = `
    ...
    uniform vec2 u_mouse; // Souris normalisée (-1 à 1)
    ...
    float fbm(vec3 p) {
        ...
        float amp = 0.5;
        float freq = 1.0;
        for(int i=0; i<4; i++) { // Reduced complexity
            f += amp * snoise(p * freq);
            p.xy *= 1.6; 
            p.z *= 1.1;
            amp *= 0.5;
    ...
    void main() {
        ...
        // CAMERA MOVEMENT: Controlled by Mouse & Scroll
        // Mouse influence on camera position
        float camX = u_mouse.x * 2.0; 
        float camY = u_mouse.y * 1.0;

        vec3 ro = vec3(camX, 0.08 + u_scroll * 3.0 + camY, -12.0); 
        ...
        for(int i=0; i<80; i++) {
            ...
            if(distToPlane < 0.5 && r > isco && r < diskRad) {
                ...
                // Add time movement + slight mouse distortion on noise
                float noiseVal = fbm(vec3(rad * 2.0, angle * 3.0 + u_time * rotSpeed * 0.2, p.y * 8.0 + u_mouse.x));
...
const BlackHoleBackground = ({ theme, scrollYProgress }: { theme: string, scrollYProgress: MotionValue<number> }) => {
  ...
  const targetMouseRef = useRef({ x: 0, y: 0 }); // Pour lissage
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-start pt-32 md:justify-center md:pt-0 px-4 z-20 pointer-events-none">
          <div className="text-center max-w-[95vw] md:max-w-7xl flex flex-col items-center pointer-events-auto">
            ...
            <motion.div
              ...
              className="w-full"
            >
              <p className="text-sm md:text-xl text-gray-700 dark:text-gray-300 max-w-lg mx-auto font-bold drop-shadow-md px-4">
                {t('hero_subtitle')}
              </p>
            </motion.div>
            ...
            >
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="group flex items-center gap-3 px-8 py-4 bg-white/10 backdrop-blur-xl border border-white/20 text-black dark:text-white rounded-full text-sm font-bold tracking-widest uppercase transition-all duration-300 hover:bg-white hover:text-black hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] z-30 cursor-pointer"
              >
