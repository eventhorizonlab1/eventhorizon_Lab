...
const ArticleCard: React.FC<{ article: Article }> = ({ article }) => {
  ...
  return (
    ...
    >
        <div className="group cursor-pointer h-full flex flex-col p-8 bg-gray-50 dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 hover:border-black dark:hover:border-white transition-colors duration-300 relative overflow-hidden">
          ...
          {/* Footer */}
          <div className="mt-8 pt-4 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-black dark:text-white group-hover:translate-x-2 transition-transform duration-300">
...
const ArticleSection: React.FC = () => {
  ...
  return (
    <motion.section 
      ...
      viewport={{ once: true }}
    >
      <div className="px-4 md:px-12 max-w-[1800px] mx-auto mb-16 flex flex-col md:flex-row justify-between items-end gap-8">
        <div>
        ...
        <div className="hidden md:flex gap-2">
          <button onClick={() => scroll('left')} className="p-4 border border-gray-200 dark:border-gray-700 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black text-black dark:text-white transition-colors">
            <ArrowLeft size={20} />
          </button>
          <button onClick={() => scroll('right')} className="p-4 border border-gray-200 dark:border-gray-700 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black text-black dark:text-white transition-colors">
            <ArrowRight size={20} />
          </button>
        </div>
      </div>
